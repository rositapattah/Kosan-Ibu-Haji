...
<div class="p-6">
    <div class="bg-white rounded-xl shadow-lg p-6">
        ...
        <form action="{{ route('kamar.update', $kamar->id) }}" method="POST" class="space-y-5">
            ...
            {{-- Status --}}
            <div>
                <label for="status_kamar" class="block text-sm font-semibold text-gray-700 mb-1">
                    Status
                </label>
                <select name="status_kamar" id="status_kamar"
                        class="block w-full rounded-lg border border-gray-300 bg-gray-50 px-4 py-2 text-gray-700 shadow-sm focus:border-indigo-400 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 transition">
                    <option value="tersedia" {{ old('status_kamar', $kamar->status_kamar) === 'tersedia' ? 'selected' : '' }}>Tersedia</option>
                    <option value="penuh" {{ old('status_kamar', $kamar->status_kamar) === 'penuh' ? 'selected' : '' }}>Terisi</option>
                </select>
            </div>
